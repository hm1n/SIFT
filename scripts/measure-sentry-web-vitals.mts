...
/**
 * Chromium을 띄운 뒤의 모든 작업을 `try`로 감싸고 `finally`에서 닫습니다.
 *
 * 닫기를 성공 경로에만 두면 서버가 떠 있지 않거나 Playwright 호출 하나가 던질 때 브라우저가 살아
 * 남습니다. 자식 프로세스와 전송 소켓이 Node의 이벤트 루프를 붙들어서, 스크립트가 약속한 종료
 * 코드를 내는 대신 그대로 매달립니다. 매달린 것은 실패로 보이지 않고 아직 도는 것으로 보이므로
 * 확인 절차가 실패를 확정하지 못하게 됩니다. 실측으로 확인했습니다. 닫혀 있는 포트를 대상으로
 * 돌리면 오류를 출력한 뒤 종료하지 않았습니다.
 */
async function main() {
  const items: CapturedItem[] = [];
