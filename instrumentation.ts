 ...
 *
 * 초기화와 전송은 `src/lib/sentry/server.ts`에 있습니다. 이 파일은 import만으로 SDK를 초기화하는
 * 부수 효과 파일이라 테스트가 로드할 수 없습니다. 이슈 #81의 제약을 이어받아 검증이 필요한 로직을
 * 전부 그쪽에 두고 여기서는 호출만 합니다. `instrumentation-client.ts`가 같은 구조입니다.
 *
 * 런타임을 가르지 않습니다. `@sentry/nextjs`가 번들러 조건(`node`, `edge-light`)에 따라 알맞은 빌드로
 * 해소되므로 nodejs와 edge에 같은 초기화를 쓰면 됩니다. Next.js가 `register()`를 런타임마다 한 번씩
 * 부르고 브라우저에서는 부르지 않으므로 `NEXT_RUNTIME`을 따로 볼 이유가 없습니다.
 */
