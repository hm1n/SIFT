 ...
 *
 * `pendingTurnIds`는 앞선 턴에서 저장이 실패해 아직 저장되지 않은 턴입니다. 클라이언트가 그 턴의
 * 식별자만 들고 있다가 다음 요청에 함께 보냅니다. 본문을 다시 실어 보내지 않아도 되는 것은 블록 갱신
 * 요청이 이미 인터뷰 전체 턴을 싣고 오기 때문입니다(`EXPERIENCE_BLOCK_HISTORY_MAX_TURNS`).
 *
...
export function turnsToSave(
  ...
  const wanted = new Set([...pendingTurnIds, answerTurnId]);
  return history
