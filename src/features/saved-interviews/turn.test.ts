...
describe("저장할 턴 고르기", () => {
  ...
  it("밀린 턴을 함께 담는다", () => {
    expect(turnsToSave(HISTORY, "t3", ["t1"]).map((message) => message.text)).toEqual([
      "질문 1",
      "답변 1",
      "질문 3",
      "답변 3",
  ...
  it("식별자 목록의 순서가 아니라 이력에 실려 온 순서를 따른다", () => {
    expect(turnsToSave(HISTORY, "t3", ["t2", "t1"]).map((message) => message.text)).toEqual([
      "질문 1",
      "답변 1",
      "질문 2",
      "답변 2",
  ...
  it("이력에 없는 턴은 담지 않는다", () => {
    expect(turnsToSave(HISTORY, "t1", ["없는턴"])).toHaveLength(2);
  });
