...
import { GitHubIcon, SiftMark } from "@/components/shell/sift-mark";
...
/** 새 탭이나 창으로 여는 클릭입니다. 이 화면은 그대로 남으므로 인증 중 상태로 바꾸지 않습니다. */
 ...
 * 기본 화면이 Empty 역할이고, 버튼을 누른 뒤 브라우저가 로그인 라우트로 이동하기 전까지가 Loading(AUTHENTICATING),
 * `auth_error` 쿼리가 있으면 Error(ERROR / AUTH)입니다. Loading과 Error는 공용 `StatusScreen`으로 그립니다.
 *
 * 이동은 `<a href>`에 맡기고 클릭에서는 상태만 바꿉니다. 내부 경로에 `window.location.assign`을 쓰는 것은 lint가 막습니다.
 */
export function LoginScreen({ authError }: LoginScreenProps) {
  ...
  function handleLoginClick(event: MouseEvent<HTMLAnchorElement>) {
    if (opensInCurrentTab(event)) setIsAuthenticating(true);
  }
    ...
    <div className={styles.screen}>
      <div className={styles.card}>
        ...
        <div className={styles.actions}>
          <ButtonLink variant="primary" className={styles.login} href={LOGIN_PATH} onClick={handleLoginClick}>
            <GitHubIcon size={16} />
