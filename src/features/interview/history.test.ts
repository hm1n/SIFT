...
describe("buildLastOutcome (구현검토 2026-09-11 P1-5)", () => {
  it("반영 성공 + provided + 충돌 없음은 눈에 띄지 않아 null이다", () => {
    expect(buildLastOutcome({ ok: true, targetResponse: "provided" }, [])).toBeNull();
  });

  it("블록 갱신 실패는 눈에 띈다", () => {
    expect(buildLastOutcome({ ok: false, targetResponse: null }, [])).toEqual({
  ...
  it("provided가 아닌 응답은 눈에 띈다", () => {
    expect(buildLastOutcome({ ok: true, targetResponse: "not_done" }, [])).toEqual({
      blockUpdateFailed: false,
      targetResponse: "not_done",
      conflicts: [],
    });
  });

  it("충돌이 있으면 눈에 띈다", () => {
    const result = buildLastOutcome({ ok: true, targetResponse: "provided" }, [{ observation: "관찰1" }]);
    expect(result).toEqual({ blockUpdateFailed: false, targetResponse: "provided", conflicts: [{ observation: "관찰1" }] });
  });

  it("충돌은 최근 것부터 상한 개수만 남긴다", () => {
    const conflicts = Array.from({ length: INTERVIEW_LAST_OUTCOME_MAX_CONFLICTS + 2 }, (_, i) => ({
      observation: `관찰${i}`,
    }));
    const result = buildLastOutcome({ ok: true, targetResponse: "unknown" }, conflicts);
    expect(result?.conflicts).toHaveLength(INTERVIEW_LAST_OUTCOME_MAX_CONFLICTS);
    expect(result?.conflicts).toEqual(conflicts.slice(-INTERVIEW_LAST_OUTCOME_MAX_CONFLICTS));
  });

  it("관찰 문장은 코드 포인트 경계에서 바이트 상한까지 자른다", () => {
    const long = "가".repeat(INTERVIEW_LAST_OUTCOME_OBSERVATION_MAX_BYTES); // 한 글자 3바이트
    const result = buildLastOutcome({ ok: true, targetResponse: "unknown" }, [{ observation: long }]);
    const bytes = new TextEncoder().encode(result!.conflicts[0].observation).byteLength;
