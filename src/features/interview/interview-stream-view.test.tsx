...
describe("InterviewStreamView 실제 생성 경로", () => {
  ...
  describe("답변 입력과 대화 누적", () => {
    ...
    it("이력에서 앞부분이 빠지면 빠진 자리에 무엇이 빠졌는지 알린다", async () => {
      ...
      const notice = screen.getByText(/drops 1 question-and-answer pair/);
      expect(notice).toHaveTextContent("drops 1 question-and-answer pair starting here");
      expect(notice).toHaveTextContent("the AI no longer sees it");
