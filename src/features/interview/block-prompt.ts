...
const RULES = [
  ...
  "평가 규칙: 주장이 바뀐 블록마다 evaluation 항목을 냅니다. targetBlock은 주장이 바뀌지 않았어도 반드시 평가합니다. 평가는 최신 답변만이 아니라 그 블록의 현재 주장 전체를 보고 판정합니다. 답변에 새 정보도 정정도 없으면 targetBlock의 평가는 입력에 있는 이전 평가와 같은 값을 그대로 냅니다. sufficient는 블록 목적의 두 요소가 모두 구체적으로 채워졌을 때만 true이고 그때 reason은 \"sufficient\" 또는 \"none\"입니다. 문장이 있다는 이유로 true가 되지 않습니다. askable은 아직 확인할 구체적인 내용이 있고 사용자가 답할 여지가 있는지입니다. reason은 sufficient, askable(더 물을 것이 있음), unknown(기억나지 않음), not_done(미실시·미측정), refused(답변 거절), none(더 물을 것이 없음) 중 하나입니다. \"대안을 비교하지 않았다\", \"성과를 측정하지 않았다\"는 유효한 진술로 주장에 남기고 not_done으로 평가합니다. 결과 블록의 두 요소는 수치를 요구하지 않습니다. 관찰한 변화와 확인한 방법이 정성적이어도(예: \"개발 화면에서 눈으로 확인했다\") 둘 다 구체적으로 있으면 수치가 없다는 이유만으로 not_done으로 낮추지 않습니다. not_done은 관찰 자체가 없거나(\"아직 실행해 보지 않았다\") 확인 방법을 특정할 수 없는 경우에 씁니다. 종료 여부나 다음 블록은 결정하지 않습니다.",
