...
const FOLLOW_UP_RULES = {
  ...
  /**
   * 이슈 #90: 질문마다 겨냥하는 블록과 요소를 프롬프트에 담습니다(설계는
   * `llm-wiki/wiki/2026-09-11-PAAR-경험블록-설계-개정.md` 6절). 값 자체는 시스템 프롬프트가 아니라
   * 별도 `focus` 메시지에 실립니다(질문마다 달라져 시스템 프롬프트 바이트 상한 유도가 흔들리지
   * 않아야 하기 때문입니다). 여기서는 그 메시지를 어떻게 쓸지만 고정된 문장으로 지시합니다.
   */
  focusTarget:
    "입력의 focus가 이번 질문이 겨냥할 블록과 요소를 가리킵니다. 이번 질문은 그 요소를 확인하는 데 집중합니다. 사용자가 자진해서 다른 블록이나 요소를 말하는 것은 막지 않지만 먼저 묻지 않습니다.",
  /**
   * 이슈 #90 구현검토(2026-09-11) P1-5: 대화 원문만으로는 블록 갱신이 내린 판단(충돌, 반영
   * 성패, 답변 분류)을 알 수 없어 질문이 이미 어긋난 전제 위에 서는 사례가 있었습니다(EventSource
   * 충돌을 사실처럼 전제하고 다음 질문을 만든 것, 사용자가 배제한 측정을 다시 물은 것). `focus`와
   * 같은 방식으로 별도 `lastOutcome` 메시지에 그 판단을 싣고, 여기서는 그 메시지를 어떻게 쓸지만
   * 지시합니다.
   */
  lastOutcomeAwareness:
    "입력의 lastOutcome이 있으면 참고합니다. blockUpdateFailed가 참이면 방금 답변이 아직 반영되지 않았을 수 있으니 그 내용을 이미 확정된 사실처럼 전제하지 않습니다. targetResponse가 not_done·unknown·refused·unanswered면 그 초점에 대해 사용자가 이미 답할 수 없거나 하지 않았다고 밝힌 것이니 같은 것을 다시 사실인 양 묻지 않습니다. conflicts가 있으면 그 관찰과 어긋나는 사용자 진술을 사실로 전제하지 말고, 필요하면 그 차이를 사용자에게 직접 확인하는 질문을 만듭니다.",
} as const;
