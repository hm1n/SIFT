...
function seeded(): ExperienceBlockState {
  const result = applyBlockUpdate(emptyExperienceBlockState(), {
    ...
    evaluation: [
      { block: "problem", sufficient: true, askable: false, reason: "sufficient" },
      { block: "action", sufficient: false, askable: true, reason: "askable" },
    ],
    targetResponse: "provided",
  } satisfies BlockUpdateOutput, { snapshot, turnId: "t1", targetBlock: "problem" });
...
describe("applyBlockUpdate", () => {
  it("추가한 주장에 순번 ID를 주고 같은 응답의 표시 문장이 new:tempId로 참조한다", () => {
    ...
    expect(state.evaluation.action).toEqual({ sufficient: false, askable: true, reason: "askable" });
    expect(state.evaluation.result).toBeNull();
  });

  it("빈 연산과 빈 표시는 삭제가 아니다. 기존 주장과 표시와 평가를 유지하고 버전만 올린다", () => {
    const state = seeded();
    const result = applyBlockUpdate(
      state,
      {
        ops: [],
        display: [],
        // targetBlock은 주장이 바뀌지 않았어도 매번 평가받아야 하므로, 새 정보가 없을 때는 모델이
        // 이전과 같은 값을 그대로 다시 냅니다(설계 4-4절).
        evaluation: [{ block: "problem", sufficient: true, askable: false, reason: "sufficient" }],
        targetResponse: "unanswered",
      },
      { snapshot, turnId: "t2", targetBlock: "problem" }
    );
    ...
    expect(result.ok && result.affectedBlocks).toEqual(["problem"]);
  });
  ...
  it("표시 문장이 있던 블록의 주장이 바뀌었는데 새 표시 문장이 없으면 거절하고, 표시가 비어 있던 블록은 주장만 쌓이는 것을 허용한다", () => {
    ...
    const fresh = applyBlockUpdate(
      seeded(),
      {
        ops: [{ op: "add", tempId: "r", block: "result", text: "첫 조각이 바로 보였습니다.", sources: [USER] }],
        display: [],
        evaluation: [{ block: "result", sufficient: false, askable: true, reason: "askable" }],
        targetResponse: "provided",
      },
      { snapshot, turnId: "t2", targetBlock: "result" }
    );
  ...
  it("미해소 충돌을 참조한 표시 문장은 그 문장만 빼고 경고로 남긴다. 응답 전체를 거절하지 않는다", () => {
    const result = applyBlockUpdate(
      ...
      { snapshot, turnId: "t2", targetBlock: "action" }
    );
    expect(result.ok).toBe(true);
    if (!result.ok) return;
  ...
  it("철회된 주장을 다시 고치거나 철회하면 거절한다", () => {
    const state = seeded();
    const retracted = applyBlockUpdate(
      state,
      {
        ops: [{ op: "retract", claimId: "c2" }],
    ...
    const result = applyBlockUpdate(
      retracted.state,
      {
        ops: [{ op: "revise", claimId: "c2", text: "다시", sources: [USER] }],
        display: [{ block: "problem", sentences: [] }],
        evaluation: [{ block: "problem", sufficient: false, askable: true, reason: "askable" }],
        targetResponse: "provided",
      },
      { snapshot, turnId: "t3", targetBlock: "problem" }
    );
  ...
  it("근거에 없는 커밋과 파일, 출처 없는 주장, 중복 임시 ID를 거절한다", () => {
    const result = applyBlockUpdate(
      ...
      { snapshot, turnId: "t1", targetBlock: "action" }
    );
  ...
  it("sufficient와 reason이 어긋나거나 알 수 없는 사유면 거절한다", () => {
    const bad = (evaluation: unknown) =>
      applyBlockUpdate(
        seeded(),
        { ops: [], display: [], evaluation: [evaluation], targetResponse: "provided" },
        { snapshot, turnId: "t2", targetBlock: "result" }
      );
  ...
  it("주장 상태가 네 블록 상한을 넘으면 거절한다", () => {
    ...
    const result = applyBlockUpdate(
      ...
      { snapshot, turnId: "t1", targetBlock: "problem" }
    );
    expect(!result.ok && result.errors[0].kind).toBe("claims_too_large");
  });

  it("targetBlock의 평가가 빠지면 주장이 바뀌지 않았어도 invalid_evaluation으로 거절한다", () => {
    const result = applyBlockUpdate(
      seeded(),
      { ops: [], display: [], evaluation: [], targetResponse: "unanswered" },
      { snapshot, turnId: "t2", targetBlock: "action" }
    );
    expect(!result.ok && result.errors.map((e) => e.kind)).toEqual(["invalid_evaluation"]);
  });

  it("이번에 주장이 바뀐 블록의 평가가 빠지면 targetBlock과 무관하게 invalid_evaluation으로 거절한다", () => {
    const result = applyBlockUpdate(
      seeded(),
      {
        ops: [{ op: "add", tempId: "r", block: "result", text: "첫 조각이 바로 보였습니다.", sources: [USER] }],
        display: [{ block: "result", sentences: [{ text: "첫 조각이 바로 보였습니다.", claimIds: ["new:r"] }] }],
        // targetBlock(problem)만 평가하고, 이번에 주장이 바뀐 result 블록의 평가를 빠뜨렸습니다.
        evaluation: [{ block: "problem", sufficient: true, askable: false, reason: "sufficient" }],
        targetResponse: "provided",
      },
      { snapshot, turnId: "t2", targetBlock: "problem" }
  ...
  it("targetResponse가 없거나 알 수 없는 값이면 거절한다", () => {
    const missing = applyBlockUpdate(
      seeded(),
      { ops: [], display: [], evaluation: [{ block: "problem", sufficient: true, askable: false, reason: "sufficient" }] },
      { snapshot, turnId: "t2", targetBlock: "problem" }
    );
    expect(!missing.ok && missing.errors.map((e) => e.kind)).toEqual(["invalid_target_response"]);

    const unknown = applyBlockUpdate(
      seeded(),
      {
        ops: [],
        display: [],
    ...
    expect(!unknown.ok && unknown.errors.map((e) => e.kind)).toEqual(["invalid_target_response"]);
  });
});
...
describe("markDisplay와 blockConflicts", () => {
  ...
  it("충돌한 주장은 revise로 해소되어 다시 active가 되고 충돌 목록에서 빠진다. new: 참조로 같은 응답에 추가한 주장에도 충돌을 걸 수 있다", () => {
    const conflicted = applyBlockUpdate(
      seeded(),
      {
        ops: [
    ...
    const resolved = applyBlockUpdate(
      conflicted.state,
      {
        ops: [{ op: "revise", claimId: "c4", text: "fetch로 본문을 스트림으로 읽었습니다.", sources: [USER] }],
        display: [{ block: "action", sentences: [{ text: "fetch로 본문을 스트림으로 읽었습니다.", claimIds: ["c4"] }] }],
        evaluation: [{ block: "action", sufficient: false, askable: true, reason: "askable" }],
        targetResponse: "provided",
      },
      { snapshot, turnId: "t3", targetBlock: "action" }
    );
    ...
    const twice = applyBlockUpdate(
      conflicted.state,
      {
        ops: [{ op: "conflict", claimId: "c4", observation: "다시" }],
        display: [{ block: "action", sentences: [] }],
        evaluation: [{ block: "action", sufficient: false, askable: true, reason: "askable" }],
        targetResponse: "provided",
      },
