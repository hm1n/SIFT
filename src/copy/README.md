...
| `shared.ts` | 두 화면 이상이 함께 쓰는 문구 |

화면별로 나누면 같은 문구가 두 파일에 각각 들어가기 쉽습니다. 한쪽만 고치면 두 화면이 조용히 갈리므로, 두 화면 이상이 쓰는 문구는 `shared.ts`에 한 번만 정의하고 각 화면 파일이 참조합니다. **담는 기준은 "같은 것을 가리키는가"이지 "글자가 같은가"가 아닙니다.** 뜻이 다른데 글자가 우연히 같은 문구는 각자의 파일에 남깁니다.
