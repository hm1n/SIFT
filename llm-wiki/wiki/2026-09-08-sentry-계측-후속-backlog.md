...
## 23. `next.config.test.ts`의 소스맵 테스트가 전체 실행에서 시간 초과로 깨짐

`next.config.test.ts`의 `브라우저 소스맵 생성을 켠다`가 전체 스위트를 돌릴 때 가끔 5초 제한을
넘겨 실패합니다. PR #153 작업 중 전체 실행 8회에서 2회 깨졌습니다. 단독 실행은 3회 모두 통과했고
1.3초, 1.0초, 0.9초였습니다. 실패한 실행의 이 파일 소요 시간은 5.9초였습니다.

단정 실패가 아니라 시간 초과입니다. 이 테스트는 `next.config.ts`를 실제로 불러오고 Sentry 래퍼가
설정을 만드는 것까지 기다리는데, 다른 파일이 동시에 도는 동안에는 5초를 넘깁니다. 기본 제한이
5초라 부하가 걸린 기기에서 먼저 걸립니다.

고치는 방법 후보는 셋입니다. 이 테스트에만 제한을 늘리는 것, 설정 로딩을 파일 단위로 한 번만
하도록 묶는 것, 이 파일을 다른 파일과 나란히 돌리지 않는 것입니다. 무엇을 고를지는 실제 소요
시간의 분포를 보고 정합니다. PR #153의 변경(`copy/shell.ts`, `app/icon.*`)은 `next.config.ts`를
건드리지 않아 원인과 무관합니다.
