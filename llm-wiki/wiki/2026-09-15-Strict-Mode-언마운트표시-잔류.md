...
두 번째는 2026-09-15에 `feature-persistence` 워크트리에서 고쳤습니다. 사용자 지시로 그 워크트리의 `.env`에만 같은 값으로 `OPENAI_API_KEY` 줄을 더했고, 값은 셸에 출력하지 않았습니다. 다른 워크트리와 메인 체크아웃의 `.env`는 건드리지 않았으므로 거기서 개발 서버를 띄우면 같은 증상이 다시 납니다.

이 어긋남이 실제로 무엇을 망가뜨렸는지는 뒤에 드러났습니다. 저장이 블록 갱신 요청에 얹혀 가므로, 인증이 실패하는 동안 인터뷰 두 개의 대화가 한 줄도 저장되지 않았습니다. 경위는 `wiki/2026-09-15-인터뷰-저장-후속-backlog.md` 7번에 있습니다.
